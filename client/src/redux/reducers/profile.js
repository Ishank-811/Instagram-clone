const reducer = (state = { myprofile:[] , userprofile:[] , follow:[] }, action) => {
  switch (action.type) {
    ...
    case "USER_PROFILE":
     ...
     case "REMOVE_USER_PROFILE":
      ...
      localStorage.setItem("profile", JSON.stringify(existing)); 
        return {...state ,  follow:state.userprofile["1"]["0"].followers.map((post)=>
          post._id===action.payload.followers._id?action.payload.followers:post )}
